...
class StoreSerenazgoRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        if ($this->dni) {
            $existing = Serenazgo::where('dni', $this->dni)->first();
            if ($existing && !$existing->activo) {
                $isReactivation = true;
            }
        }

        $dniRule = $isReactivation
            ? 'digits:8'
            : ['digits:8', Rule::unique('serenazgos', 'dni')];

        return [
            'nombres' => 'required|string|max:255',
            'apellido_paterno' => 'required|string|max:255',
            'apellido_materno' => 'required|string|max:255',
            'dni' => array_merge(['required'], is_array($dniRule) ? $dniRule : [$dniRule]),
